    ...
    <StaggerGroup className="mb-4 grid gap-3 lg:grid-cols-3">
      {announcements.length > 0 && (
        <MotionStaggerItem className="lg:col-span-3">
          <ThreeDCardContainer maxDegrees={6} className="rounded-3xl border border-amber-500/30 bg-white dark:bg-transparent p-4 backdrop-blur-xl shadow-xl">
            <div className="mb-2 flex items-center gap-2 text-sm font-bold text-amber-600 dark:text-amber-300">
              <Megaphone className="h-4 w-4 animate-bounce" />
              Company Announcements
            </div>
            {announcements.map((a) => (
              <div key={a.id} className="mb-2 last:mb-0">
                <p className="font-bold text-slate-900 dark:text-white text-base">{a.title}</p>
                <p className="text-sm text-slate-600 dark:text-slate-300">{a.body}</p>
              </div>
            ))}
          </ThreeDCardContainer>
        </MotionStaggerItem>
        ...
        <MotionStaggerItem key={`${c.type}-${c.employee_id}`}>
          <ThreeDCardContainer maxDegrees={8} className="rounded-3xl border border-pink-500/30 bg-white dark:bg-transparent p-4 backdrop-blur-xl shadow-xl">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-pink-100 dark:bg-pink-500/20 text-pink-600 dark:text-pink-300 border border-pink-200 dark:border-pink-500/30 shadow-md">
                {c.type === "birthday" ? (
      ...
      {streak.badge && (
        <MotionStaggerItem>
          <ThreeDCardContainer maxDegrees={10} className="rounded-3xl border border-orange-500/30 bg-white dark:bg-transparent p-4 backdrop-blur-xl shadow-xl">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-orange-100 dark:bg-orange-500/20 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-500/30 shadow-md">
              ...
              <div>
                ...
                <p className="text-base font-extrabold text-slate-900 dark:text-white">{streak.badge}</p>
              </div>
            </div>
          </ThreeDCardContainer>
        </MotionStaggerItem>
